...
public class ContractVisitor1 extends SolidityBaseVisitor<Void> {
    ...
    private Map<String, List> lineNoif = new HashMap<>();
    ...
    ArrayList<Integer> linenString = new ArrayList<Integer>();
    ArrayList<Integer> linenBytes = new ArrayList<Integer>();
    ArrayList<Integer> lineReturn = new ArrayList<Integer>();
    ArrayList<Integer> lineExCall = new ArrayList<Integer>();
    List<Integer> line = new ArrayList<Integer>();

    public ContractVisitor1(String sourceText) {
        this.sourceText = sourceText;
    }

    public ContractVisitor1() {
    }
    ...
    public Void visitFunctionDefinition(@NotNull FunctionDefinitionContext ctx) {
        ...
        if (ctx.returnParameters() != null) {
            lineReturn.add(ctx.start.getLine());
            countReturn++;
    ...
    public Void visitElementaryTypeName(@NotNull ElementaryTypeNameContext ctx) {
        ...
        if (ctx.getText().contains("string")) {
            nString++;
            linenString.add(ctx.start.getLine());
            metric.put("nString", nString);
        }
        if (ctx.getText().contains("bytes")) {
            linenBytes.add(ctx.start.getLine());
            nBytes++;
    ...
    public Void visitFunctionCallArguments(@NotNull FunctionCallArgumentsContext ctx) {
        if (ctx.getParent() instanceof ExpressionContext) {
            ...
            noif.add(a);
            if (lineNoif.containsKey(a)) {
                lineNoif.get(a).add(ectx.start.getLine());
            } else {
                List line1 = new ArrayList();
                line1.add(ectx.start.getLine());
                lineNoif.put(ectx.getChild(0).getText().toString(), line1);
            }
        }
        return null;
    }
    ...
    public ArrayList<Integer> getLinenString() {
        return linenString;
    }

    public ArrayList<Integer> getLinenBytes() {
        return linenBytes;
    }

    public ArrayList<Integer> getLineReturn() {
        return lineReturn;
    }

    public ArrayList<Integer> getLineExCall() {
        return lineExCall;
    }
    ...
    public Map<String, List> getLineNoif() {
        return lineNoif;
    }
}
